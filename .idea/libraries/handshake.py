...
wb_in = load_workbook(p+'user_and_team_info.csv+ (多个连接).xlsx')
ws_in = wb_in.active
team_info = {}
edges_count = 0
for i in range(2, ws_in.max_row):
    team_id = ws_in.cell(i, 3).value
    if team_info.__contains__(team_id) == False:
        team_info[team_id] = []
    team_info[team_id].append(ws_in.cell(i, 2).value)
